...
function Get-KeeperShareNeedsUpdate {
    <#
    .SYNOPSIS
        Checks if a share (record or shared folder) needs its permissions updated for a user.

    .PARAMETER Vault
        The Keeper VaultOnline context object.

    .PARAMETER User
        The email/UID of the user to check permissions for.

    .PARAMETER ShareUid
        The UID of the shared item (record UID or shared folder UID).

    .PARAMETER Elevated
        Whether the desired permissions are elevated (true = full access).

    .PARAMETER ApplicationUid
        The UID of the application record to fetch app info.

    .OUTPUTS
        Boolean. True if the share needs update, otherwise false.
    #>
    ...
    $isRecordShare = $appInfo.Shares | Where-Object {
        ($_.SecretUid -eq $ShareUid) -and
        ($_.ShareType -eq [KeeperSecurity.Vault.SecretManagerSecretType]::Record)
    }

    if ($isRecordShare) {
        $ShareUids = [System.Collections.Generic.List[string]]::new()
        $ShareUids.Add($ShareUid)
        $recordShares = $vault.GetSharesForRecords($ShareUids).GetAwaiter().GetResult()
        $recordShare = $recordShares | Where-Object { $_.RecordUid -eq $ShareUid }

        if (-not $recordShare) {
            return $false
        }

        $userPerms = $recordShare.UserPermissions | Where-Object { $_.Username -eq $User }
        if (-not $userPerms) {
            return $true
        }
        ...
        $sharedFolder = $null
        $vault.TryGetSharedFolder($ShareUid, [ref]$sharedFolder) | Out-Null
