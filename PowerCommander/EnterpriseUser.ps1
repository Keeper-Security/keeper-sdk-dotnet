...
function Add-KeeperEnterpriseUserTeam {
    <#
        .SYNOPSIS
        Adds one or more teams to an enterprise user.

        .DESCRIPTION
        Matches Commander enterprise-user --add-team. Adds the user to one or more teams
        by team name or UID. Use -HideSharedFolders (on/off) for team admin types.
        ...
        .PARAMETER Team
        One or more team names or UIDs.

        .PARAMETER HideSharedFolders
        on = Admin Only (no shared folders). off = Admin with shared folder access.
        Applies only when adding teams (--add-team).

        .EXAMPLE
        Add-KeeperEnterpriseUserTeam -User "user@example.com" -Team "Engineering", "DevOps"

        .EXAMPLE
        Add-KeeperEnterpriseUserTeam -User "user@example.com" -Team "Engineering" -HideSharedFolders on
    #>
    [CmdletBinding(SupportsShouldProcess = $true, ConfirmImpact = 'Medium')]
    Param (
        [Parameter(Position = 0, Mandatory = $true)]$User,
        [Parameter(Position = 1, Mandatory = $true)][string[]]$Team,
        [ValidateSet('on', 'off')][string]$HideSharedFolders
    )

    if ($Team.Count -eq 0) {
        Write-Error "At least one team must be specified via -Team." -ErrorAction Stop
    }
    ...
    foreach ($teamInput in $Team) {
        if ([string]::IsNullOrWhiteSpace($teamInput)) { continue }
        $teamTarget = Resolve-EnterpriseTeamTarget -Enterprise $enterprise -TeamInput $teamInput.Trim()
        if (-not $teamTarget) { continue }
...
Register-ArgumentCompleter -CommandName Add-KeeperEnterpriseUserTeam -ParameterName User -ScriptBlock $Keeper_EnterpriseUserCompleter
Register-ArgumentCompleter -CommandName Add-KeeperEnterpriseUserTeam -ParameterName Team -ScriptBlock $Keeper_TeamNameCompleter

function Remove-KeeperEnterpriseUserTeam {
    <#
        .SYNOPSIS
        Removes one or more teams from an enterprise user.

        .DESCRIPTION
        Matches Commander enterprise-user --remove-team.
        ...
        .EXAMPLE
        Remove-KeeperEnterpriseUserTeam -User "user@example.com" -Team "Engineering", "DevOps"
    #>
    [CmdletBinding(SupportsShouldProcess = $true, ConfirmImpact = 'Medium')]
    Param (
        [Parameter(Position = 0, Mandatory = $true)]$User,
    ...
    $teamUids = [System.Collections.Generic.List[string]]::new()
    foreach ($teamInput in $Team) {
        if ([string]::IsNullOrWhiteSpace($teamInput)) { continue }
        $teamTarget = Resolve-EnterpriseTeamTarget -Enterprise $enterprise -TeamInput $teamInput.Trim()
        if ($teamTarget) { [void]$teamUids.Add($teamTarget.Uid) }
    }

    if ($teamUids.Count -eq 0) {
        Write-Warning "No valid teams found to remove."
        return
    }

    if ($PSCmdlet.ShouldProcess("Remove user `"$($userObject.Email)`" from $($teamUids.Count) team(s)", 'Remove')) {
        try {
            ...
            Write-Output "Removed user '$($userObject.Email)' from $($teamUids.Count) team(s)."
        }
        catch {
            Write-Error "Failed to remove user '$($userObject.Email)' from team(s): $($_.Exception.Message)" -ErrorAction Stop
        }
    }
}
Register-ArgumentCompleter -CommandName Remove-KeeperEnterpriseUserTeam -ParameterName User -ScriptBlock $Keeper_EnterpriseUserCompleter
Register-ArgumentCompleter -CommandName Remove-KeeperEnterpriseUserTeam -ParameterName Team -ScriptBlock $Keeper_TeamNameCompleter
