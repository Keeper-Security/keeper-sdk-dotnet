...
public class TypedRecordFacade<T> where T : TypedRecordFields, new()
{
    ...
    {
        Fields = new T();
        TypedRecord = record ?? new TypedRecord(Fields.RecordType);

        foreach (var field in TypedRecord.Fields)
        {
            Fields.LoadTypedField(field);
        }

        TypedRecord.Fields.AddRange(Fields.CreateMissingFields());
    }

    public TypedRecord TypedRecord { get; }

    public T Fields { get; }
    public IList<ITypedField> Custom => TypedRecord.Custom;
}
...
public class PersonBirthDateRecordType : TypedRecordFileRef
{
    ...
    protected internal override void LoadTypedField(ITypedField field)
    {
        ...
        {
            case "name" when _name == null:
                _name = field as TypedField<FieldTypeName>;
                break;
            case "birthDate" when _birthDate == null:
...
public class PassportRecordType : PersonBirthDateRecordType
{
    ...
    protected internal override void LoadTypedField(ITypedField field)
    {
        switch (field.FieldName)
        {
            case "accountNumber" when field.FieldLabel == "passportNumber" && _passportNumber == null:
                _passportNumber = field as TypedField<string>;
                break;
            case "expirationDate" when _expirationDate == null:
                _expirationDate = field as TypedField<long>;
                break;
            case "date" when field.FieldLabel == "dateIssued" && _dateIssued == null:
                _dateIssued = field as TypedField<long>;
                break;
            case "password" when _password == null:
                _password = field as TypedField<string>;
                break;
            case "addressRef" when _addressRef == null:
...
public class DriverLicenseRecordType : PersonBirthDateRecordType
{
    ...
    protected internal override void LoadTypedField(ITypedField field)
    {
        switch (field.FieldName)
        {
            case "accountNumber" when field.FieldLabel == "dlNumber" && _dlNumber == null:
...
public class LoginRecordType : TypedRecordFileRef
{
    ...
    protected internal override void LoadTypedField(ITypedField field)
    {
        switch (field.FieldName)
        {
            case "login" when _login == null:
                _login = field as TypedField<string>;
                break;
            case "password" when _password == null:
                _password = field as TypedField<string>;
                break;
            case "url" when _url == null:
                _url = field as TypedField<string>;
                break;
            case "oneTimeCode" when _oneTimeCode == null:
