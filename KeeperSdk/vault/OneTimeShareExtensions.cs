        ...
        public static async Task<ExternalRecordShare[]> GetExernalRecordShares(this VaultOnline vault, string recordUid)
        {
            ...
            return rs.AppInfo.Where(x => x.IsExternalShare).SelectMany(share => share.Clients, (share, client) => new { share, client })
                .Select(x => new ExternalRecordShare
                {
                    ...
                    FirstAccessed = x.client.FirstAccess > 0 ? DateTimeOffsetExtensions.FromUnixTimeMilliseconds(x.client.FirstAccess) : (DateTimeOffset?) null,
                    LastAccessed = x.client.LastAccess > 0 ? DateTimeOffsetExtensions.FromUnixTimeMilliseconds(x.client.LastAccess) : (DateTimeOffset?) null,
                }).ToArray();
        }
